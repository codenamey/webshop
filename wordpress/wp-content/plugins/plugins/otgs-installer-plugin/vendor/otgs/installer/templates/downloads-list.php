...
$download_button_disabled = ! $is_wpml || $is_wpml_installed ? ' disabled' : '';
$activate_checkbox_checked = $is_wpml && ! $is_wpml_installed ? ' checked' : '';
        ...
        <table class="widefat installer-plugins">
			...
			foreach ( reset( $sections )['downloads'] as $download_id => $download ) {
				if ( DownloadFilter::shouldDisplayRecord($download_id)) {
					$checked = $is_wpml && ! $is_wpml_active && $download_id === "sitepress-multilingual-cms";
